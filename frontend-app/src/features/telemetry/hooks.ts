...
export function useTimeseries(assetId: string | null, metric: string) {
  ...
  return useQuery({
    queryKey: ['assetMetricsTimeseries', assetId, metric, from.toISOString(), to.toISOString(), '5m'],
    queryFn: () => getTimeseries(assetId!, metric, from.toISOString(), to.toISOString(), '5m'),
    enabled: Boolean(assetId),
  })
}
