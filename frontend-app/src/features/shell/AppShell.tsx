...
const dock = [
  { id: 'engine', label: '3D Engine', icon: Cuboid, tone: 'default' },
  { id: 'layers', label: 'X-Ray Layers', icon: Layers3, tone: 'default' },
  { id: 'monitoring', label: 'Monitoring', icon: Activity, tone: 'default' },
  { id: 'alarms', label: 'Alarms', icon: BellRing, tone: 'alarm' },
  { id: 'time', label: 'Time Machine', icon: Clock3, tone: 'sim' },
  { id: 'integration', label: 'Integration', icon: Shield, tone: 'default' },
] as const
...
export function AppShell() {
  ...
  const alarmCount = useAlarms().data?.items.length ?? 0
    ...
    <div className="min-h-screen bg-[#060B12] text-[#E4EDFB]">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex h-14 items-center justify-center gap-1 border-t border-white/5 bg-[#060B12]/95 backdrop-blur-xl">
        {dock.map((item, index) => {
          ...
          const activeClass = active ? 'border-[#00D4FF]/20 bg-[#00D4FF]/10 text-[#00D4FF]' : item.tone === 'alarm' ? 'text-[#FF3D5A] hover:border-[#FF3D5A]/20 hover:bg-[#FF3D5A]/10' : item.tone === 'sim' ? 'text-[#A855F7]' : 'text-[#3D5368] hover:border-white/5 hover:bg-[#152338] hover:text-[#7A90AA]'
          return (
            <div className="contents" key={item.label}>
              {index === 1 ? <div className="mx-1 h-8 w-px bg-white/5" /> : null}
              <button className={`relative flex min-w-[72px] flex-col items-center gap-0.5 rounded-lg border border-transparent px-4 py-1.5 transition ${activeClass}`} onClick={() => setActivePanel(item.id)} type="button">
                <Icon size={18} />
                <span className="text-[9px] font-semibold uppercase tracking-[0.04em]">{item.label}</span>
                {item.id === 'alarms' && alarmCount ? <span className="absolute right-2 top-0.5 rounded-full bg-[#FF3D5A] px-1.5 text-[7px] font-bold text-white">{alarmCount}</span> : null}
              </button>
            </div>
          )
        })}
